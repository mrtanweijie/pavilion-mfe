...
**Segment（段）** 是独立的微前端应用，遵循统一的 `mount` / `unmount` 生命周期契约，可使用任意框架。段在运行时不含任何 `@pavilion-mfe/*` 代码，仅构建时依赖 `@pavilion-mfe/vite`。
...
| 包名 | 说明 | 依赖 |
|------|------|------|
| `@pavilion-mfe/bridge` | Shell-Segment 事件通信桥（EventBus + StorageSync） | 零依赖 |
...
段应用**零运行时依赖**，无需引入 `@pavilion-mfe/*` 包即可检测环境。
...
| `pavilion-mfe:segment-switch` | 活跃段发生变化 | `{ from: string[], to: string[] }` |
| `pavilion-mfe:before-cache` | 段进入 Keep-Alive 缓存前 | `{ appCode }` |
| `pavilion-mfe:after-restore` | 段从缓存恢复显示 | `{ appCode }` |
| `pavilion-mfe:segment-error` | 段加载/挂载失败 | `{ appCode, phase, error }` |

```typescript
window.addEventListener('pavilion-mfe:segment-switch', (e) => {
...
```
[PavilionMfe] router    router-start       segments=2
[PavilionMfe] router    segment-register   appCode=demo-app
[PavilionMfe] router    before-routing     trigger=pushState  url=/demo/list
[PavilionMfe] sandbox   sandbox-activate   appCode=demo-app
[PavilionMfe] router    segment-load       appCode=demo-app  ms=320
[PavilionMfe] router    segment-mount      appCode=demo-app  ms=45
...
|------|-------------------|------|
| Shell | `@pavilion-mfe/router` `@pavilion-mfe/sandbox` (runtime dep) + `@pavilion-mfe/vite` (devDep) | 运行时需要路由和沙箱 |
| Segment | `@pavilion-mfe/vite` (devDep only) | 仅构建时需要，运行时不含 `@pavilion-mfe/*` 代码 |

段应用更新 `@pavilion-mfe/vite` 版本后独立发布即可，**不影响 Shell 运行时**，无需 Shell 重新构建。
